	...
	useEffect(() => {
		if (card) {
			...
			form.reset({
				...
				reflectionMessage: card.reflectionMessage || '',
				deckId: card.deckId || null,
				summary: card.summary || '',
				description: card.description || '',
				generalReading: generalType?.read || '',
				loveReading: loveType?.read || '',
				careerReading: careerType?.read || '',
				spiritualReading: spiritualType?.read || '',
				invertedReading: invertedType?.read || '',
				verticalMeanings: card.verticalMeaning as string[],
				invertedMeanings: card.invertedMeaning as string[],
			})
		}
	}, [card, form])
			...
			>
				...
				<div className="lg:sticky lg:top-24 lg:self-start space-y-4">
					...
					<div className="rounded-xl border-2 border-violet-500/30 bg-gradient-to-br from-violet-500/10 via-purple-500/10 to-indigo-500/10 p-5 backdrop-blur-sm shadow-lg shadow-violet-500/10">
						...
						>
							{({ name, cardType, suit, numerology, astrology }) => (
