...
type SearchMatch = {
	field: 'nome' | 'numerologia' | 'astrologia' | 'tag' | 'significado' | 'descricao'
	value: string
}
...
export function GlobalSearch({ open, onClose }: GlobalSearchProps) {
	...
	const filterCards = useCallback((cards: TarotCardFromRouter[] | undefined) => {
		...
		return cards.filter((card) => {
			...
			if (Array.isArray(verticalMeanings) && verticalMeanings.some((meaning) =>
			...
			if (Array.isArray(invertedMeanings) && invertedMeanings.some((meaning) =>
				typeof meaning === 'string' && meaning.toLowerCase().includes(searchTerm),
			)) {return true}
			...
			if (card.summary?.toLowerCase().includes(searchTerm)) {
				matches.push({ field: 'descricao', value: card.summary })
			}
			...
			<div className="flex h-full w-full items-center justify-center relative px-4" onClick={handleBackgroundClick}>
				<div className="flex flex-col items-center w-full max-w-2xl" onClick={(e) => { e.stopPropagation() }}>
									...
									? (
										<div className="flex flex-col items-center justify-center py-16 text-center">
											...
											<p className="mt-2 text-sm text-muted-foreground/80 leading-relaxed max-w-xs">
												O oraculo nao revela segredos com este termo. Tente outro caminho.
											</p>
										</div>
									)
									: (
																		...
																		<div className="flex items-center gap-1 flex-wrap">
																			{result.matches.slice(0, 2).map((match, i) => {
																				const iconMap = {
																					...
																					tag: 'lucide:tag',
																					nome: 'lucide:tag',
																					descricao: 'lucide:sparkles',
																				} as const

																				const iconString = iconMap[match.field] || 'lucide:tag'
					...
					<p className="mt-4 text-xs text-muted-foreground/50 text-center">
						O oraculo revela seus segredos aos que buscam com intencao pura
					</p>
				</div>
			</div>
		</div>
